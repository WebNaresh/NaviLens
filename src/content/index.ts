...
// --- Gemini Auto-Paste Logic ---

if (window.location.hostname.includes('gemini.google.com')) {
    console.log('[Content] Detected Gemini - Checking for pending paste...');
    
    // Check for pending paste
    chrome.storage.local.get('pending_gemini_paste', async (result: any) => {
        const pasteData = result['pending_gemini_paste'];
        
        if (pasteData && pasteData.imageUri) {
            console.log('[Content] Found pending image to paste!');
            
            // Clear flag immediately to prevent double paste
            await chrome.storage.local.remove('pending_gemini_paste');

            // Wait for input to be ready
            const checkForInput = setInterval(async () => {
                const inputArea = document.querySelector('div[contenteditable="true"]');
                if (inputArea) {
                    clearInterval(checkForInput);
                    console.log('[Content] Found input area, simulating paste...');
                    
                    try {
                        // Create Blob from Base64
                        const res = await fetch(pasteData.imageUri);
                        const blob = await res.blob();
                        const file = new File([blob], "captured-screenshot.png", { type: "image/png" });
                        ...
                        console.log('[Content] Paste event dispatched!');
                        
                    } catch (err) {
                        console.error('[Content] Auto-paste failed:', err);
                    }
                }
            }, 500);
